  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        }/complaints/raiseComplaint?reservationId=${reservationId}`,
        formData,
        {
          headers: {
      ...
      <div className="bg-blue-50 p-4 rounded-lg shadow-md mx-4 sm:mx-10 md:mx-20 lg:mx-40 mt-11">
        ...
        <div className="flex space-x-4">
          {photos.map((photo, index) => (
            <div
              key={index}
              className="w-24 h-24 border-2 border-dashed border-gray-400 rounded-lg flex items-center justify-center bg-gray-200"
            >
              <img
